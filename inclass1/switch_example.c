...
//
//The PORTD pin that exhibits the "most" bounce (0-7) : from my observations it seems that PORTD pin 0 has the most
//	bounce. At 125ns none of my switches bounced more than twice however, pin 0 bounced with the most frequencey
//	comapred to the others. 
//
//
//With switch debouncer enabled:
//  too fast, bounces counted            :   Bounces were observed at 125ns (ns or mS)
//
//  too slow, misses fast button pushes  :   32.7ms button pushes were missed completley  (ns or mS)
//
//Without switch debouncer:
//  too fast, bounces counted            :   Bounces were counted at 125ns up to 32.7ms (ns or mS)
//
//  too slow, misses fast button pushes  :   Without the switch debouncer I was unable to press the switches fast enough for the program to miss them even at 32.7 ms (ns or mS)
...
ISR(TIMER0_COMP_vect){
  ...
  switch(button_state){
    case(NOTSET): 
      if(bit_is_clear(PIND,6)){ //stay here till button is pushed
        button_state=SET;
      } break;
    case(SET): 
      ...
      break;
    case(WAIT): 
      if(bit_is_set(PIND,6)){   //stay here till button is released
        button_state=NOTSET;
      } 
      break;
    default: break;
