...
void spi_init(void){
  ...
  DDRB   = 0b00000111;  //output mode for SS, MOSI, SCLK
...
while(1){                             //main while loop
    ...
    PORTB = PORTB ^ 0x07;             //send falling edge to regclk on 74HC595

    display_count = (display_count << 1);
    
    if(display_count == 0x80) {display_count = 1;}  //shift display_count for next time 
